...
SELECT *
FROM countries
WHERE 56 <= life_expectancy AND life_expectancy <= 76;
...
-- 「Arubaの人口は103000人です」
SELECT CONCAT(region , 'の人口は', population , 'です') AS region_info
FROM countries;
...
SELECT countries.name AS '国名', countrylanguages.language
FROM countries
JOIN countrylanguages ON countries.code = countrylanguages.country_code;

-- 問26
-- 全ての国と言語と市区町村を表示してください。
SELECT countries.name AS '国名', cities.district AS '市区町村名', countrylanguages.language
FROM countries
...
SELECT c.name AS celebrity_name, 
       (SELECT co.name FROM countries co WHERE co.code = c.country_code) AS country_name
FROM celebrities c;

-- 問30
-- 最年長が50歳以上かつ最年少が30歳以下の国を表示させてください。
SELECT countries.name AS country_code,MAX(celebrities.age) AS oldest_age,MIN(celebrities.age) AS youngest_age
FROM celebrities
